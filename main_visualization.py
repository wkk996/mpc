...
# 加载不同控制器的路径数据
# ---------------------------
with open("results/data_test_CBF-MPC.txt", 'rb') as file:
    data = pickle.load(file)
path0 = data['path']
ref = data['ref']

with open("results/data_test_Myopic MPC.txt", 'rb') as file:
    data = pickle.load(file)
path1 = data['path']
ref = data['ref']

with open("results/data_test_ampc.txt", 'rb') as file:
    data = pickle.load(file)
path2 = data['path']
# 从AMPC数据中获取 inside_obstacle_points（如果存在）
inside_obstacle_points = data.get('inside_obstacle_points', [])

with open("results/data_test_SANMPC.txt", 'rb') as file:
    data = pickle.load(file)
path3 = data['path']
# ---------------------------
# 定义障碍物，位置和半径
...
legend_handles = [start_marker, end_marker, collision_marker,
                  line_CBF, line_Myopic, line_AMPC,line_SAN]
...
plt.savefig('quadrotor3D11.png', dpi=300, transparent=False, facecolor='white')
# 显示图形
plt.show()
